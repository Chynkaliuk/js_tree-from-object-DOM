...
function createTree(element, data) {
    ...
    for(const key in data)if (data.hasOwnProperty(key)) {
        ...
        if (typeof nestedData === 'object' && nestedData !== null && Object.keys(nestedData).length > 0) // Рекурсивний виклик для вкладених даних, батько - поточний <li>
        createTree(li, nestedData);
    }
}
// --- ПЕРШИЙ ВИКЛИК ФУНКЦІЇ ---
// Перевіряємо, чи знайдено батьківський елемент
if (tree) // Запускаємо процес створення дерева
